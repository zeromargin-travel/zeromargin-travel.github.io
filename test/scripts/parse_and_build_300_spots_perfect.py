#!/usr/bin/env python3
"""
Zero-Margin Travel App - Perfect Dutch 300 Parser (v33.0.0)
Parses scripts/dutch_prompt_text.txt and creates 5 city JSON files:
...
def run_perfect_parser():
    with open("scripts/dutch_prompt_text.txt", "r", encoding="utf-8") as f:
        text = f.read()

    print(f"📖 Loaded {len(text)} characters of text from scripts/dutch_prompt_text.txt.")
    ...
    for fname, c_en, cnt_en, c_ja, cnt_ja, prefix, default_lat, default_lng, sec_pattern in city_configs:
        ...
        # Regex to split spots by index e.g. "1. " or "### 1. "
